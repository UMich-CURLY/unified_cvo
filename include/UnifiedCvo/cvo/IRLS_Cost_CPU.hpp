...
namespace cvo {
  ...
  class PairwiseWithOffsetAutoDiffFunctor {
  public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    
    PairwiseWithOffsetAutoDiffFunctor(const Eigen::Vector3f & pt1,
                            const Eigen::Vector3f & pt2,
                            double label_ip,
                            double ell,
                            double sigma,
                            const Eigen::Vector3f & offset_translation
                            ) {
      pt1_.head<3>() = pt1.cast<double>();
      pt1_[3] = 1.0;
      pt2_.head<3>() = pt2.cast<double>();
      ...
      sigma2_ = sigma*sigma;
      label_ip_ = label_ip;
      offset_translation_.block<3,3>(0,0) = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>::Identity();
      offset_translation_.block<3,1>(0,3) = offset_translation.cast<double>();
    }

    ~PairwiseWithOffsetAutoDiffFunctor() {}
    ...
    bool operator()(const T* pose1_vec,
      ...
      Eigen::Matrix<T, 3, 4, Eigen::RowMajor> T2_offset;
  ...
  private:
    ...
    double sigma2_;
    double label_ip_;
    Eigen::Matrix<double, 3, 4, Eigen::RowMajor> offset_translation_;
  };
