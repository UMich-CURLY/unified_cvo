...
namespace cvo {
  LyftHandler::LyftHandler(std::string lyft_folder, int data_type) {
    curr_index = 0;
    folder_name = lyft_folder;
    debug_plot = true;
    if(data_type==0){
    ...
    path lyft(data_folder.c_str());
    for (auto & p : directory_iterator(lyft)) {
    ...
    cout<<"Lyft contains "<<names.size()<<" files\n";
  ...
  int LyftHandler::read_next_stereo(cv::Mat &left, cv::Mat &right,
  ...
  int LyftHandler::read_next_lidar_mono(cv::Mat & image,
                                         pcl::PointCloud<pcl::PointXYZ>::Ptr pc  ) {
    std::cerr<<"lidar-mono not implemented in LyftHandler\n";
    assert(0);
    return -1;
  }

  int LyftHandler::read_next_lidar(pcl::PointCloud<pcl::PointXYZI>::Ptr pc  ) {

    if (curr_index >= names.size())
      return -1;
    
    string lidar_bin_path = data_folder + names[curr_index] + ".bin";
    ...
    int num_lidar_points = fLidar_length / (5 * sizeof(float));

    std::vector<float> lidar_points;
    
    if (fLidar.is_open()){
      int num_bytes = sizeof(float) * 5 * num_lidar_points;
      
      lidar_points.resize(5*num_lidar_points);
      ...
      for(int r=0; r<num_lidar_points; ++r){   
          ...
          temp_pcl.x = lidar_points[r*5+0];
          temp_pcl.y = lidar_points[r*5+1];
          temp_pcl.z = lidar_points[r*5+2];
          temp_pcl.intensity = lidar_points[r*5+3]; // no use
          ...
          pcl_after_tf.intensity = lidar_points[r*5+3]; // no use
  ...
  int LyftHandler::read_next_lidar(pcl::PointCloud<pcl::PointXYZI>::Ptr pc,
    ...
    string semantic_bin_path = data_folder + names[curr_index] + ".label";
  ...
  std::map<int,int> LyftHandler::create_label_map(){
  ...
  void LyftHandler::next_frame_index() {
    curr_index ++;
  }


  void LyftHandler::set_start_index(int start) {
    curr_index = start;
    
  }

  int LyftHandler::get_current_index() {
    return curr_index;
    
  }

  int LyftHandler::get_total_number() {
    return names.size();
