...
bool eval (int seq, string gt_dir, string gt_file_name, string result_dir, string result_file_name, Mail* mail) {
  ...
  for (int32_t i=seq; i<seq+1; i++) {
    ...
    // file name
    char file_name_c_str[256];
    sprintf(file_name_c_str,"%02d.txt",i);
    string file_name(file_name_c_str);

    ///string file_name = "tmp.txt"; //result_file_name.substr(17,result_file_name.size()-17);
    string seq_str;
    ...
    std::cout<<"gt_dir: "<<gt_dir + gt_file_name<<std::endl;
    std::cout<<"result_dir: "<<result_dir + result_file_name<<std::endl;
    vector<Matrix> poses_gt     = loadPoses(gt_dir + gt_file_name);
